      ...
      <Card>
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {rows.map(r => (
                <TableRow key={r.mes} className={r.isProjecao ? 'opacity-50' : ''}>
                  <TableCell className="capitalize text-xs font-medium">{r.label}</TableCell>
                  <TableCell>
                    {r.isProjecao ? <Badge variant="outline" className="text-[9px] h-4">Projeção</Badge>
                      : r.mes === mesAtual ? <Badge className="text-[9px] h-4 bg-primary">Atual</Badge>
